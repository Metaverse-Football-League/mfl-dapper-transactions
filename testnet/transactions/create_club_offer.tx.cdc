...
transaction(
	...
	var nftReceiver: Capability<&MFLClub.Collection>
    ...
    prepare(dapp: auth(Storage, Capabilities) &Account, signer: auth(Storage, Capabilities) &Account, dapper: auth(Storage, Capabilities) &Account) {
		...
		if signer.storage.borrow<&MFLClub.Collection>(from: MFLClub.CollectionStoragePath) == nil {
			let collection <- MFLClub.createEmptyCollection(nftType: Type<@MFLClub.NFT>())
			signer.storage.save(<-collection, to: MFLClub.CollectionStoragePath)
		}

        self.nftReceiver = signer.capabilities.get<&MFLClub.Collection>(MFLClub.CollectionPublicPath)
		if !self.nftReceiver.check() {
			signer.capabilities.unpublish(MFLClub.CollectionPublicPath)
			let collectionCap = signer.capabilities.storage.issue<&MFLClub.Collection>(MFLClub.CollectionStoragePath)
			signer.capabilities.publish(collectionCap, at: MFLClub.CollectionPublicPath)
			self.nftReceiver = signer.capabilities.get<&MFLClub.Collection>(MFLClub.CollectionPublicPath)
        }
    ...
    execute {
        ...
        let typeId = "A.683564e46977788a.MFLClub.NFT"

		let offerParamsString: {String: String} = {}
        offerParamsString.insert(key: "nftId", clubId.toString())
